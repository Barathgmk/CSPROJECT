...
**Video Walkthrough:** [Click here to watch the demo video](#) *(Add your video link here)*

**GitHub Repository:** [GitHub Link](#) *(Add your GitHub repo link here)*

**Live Demo:** [Live Link](#) *(Add live deployment link if available)*
...
4. Copy your credentials into `config/.env`:
...
- **`/src`** - Python backend (api.py, mock_trader.py, simple_ml.py)
- **`/frontend`** - Web dashboard (index.html)
- **`/config`** - Configuration files (.env, requirements.txt)
- **`/data`** - Generated data files (penny_candidates.csv, etc.)
- **`/docs`** - Documentation (README, guides, architecture)

See `PROJECT_STRUCTURE.md` for detailed explanation of the directory layout.
